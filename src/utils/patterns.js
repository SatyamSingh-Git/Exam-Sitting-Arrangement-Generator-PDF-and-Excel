function expandClasses(classes) {
  // expand each class into its student list
  const perClass = classes.map(c => {
    ...
    return { group: c.group || (classes.indexOf(c) + 1), students: arr }
  })

  // merge classes that share the same group number
  const groupMap = new Map()
  for (const entry of perClass) {
    const g = entry.group
    if (!groupMap.has(g)) groupMap.set(g, [])
    groupMap.get(g).push(...entry.students)
  }

  // return one list per group (ordered by group number)
  const sortedGroups = [...groupMap.entries()].sort((a, b) => a[0] - b[0])
  return sortedGroups.map(([, students]) => students)
}
